    ...
    try:
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                # Smoothing the pose
                smoothed_landmarks = results.pose_landmarks.landmark
                for lm in smoothed_landmarks:
                    lm.x = np.clip(lm.x, 0, 1)
                    lm.y = np.clip(lm.y, 0, 1)
                # Use a colored gradient-like skeleton for readability
                mp_drawing.draw_landmarks(
                    black_background,
                    results.pose_landmarks,
                    mp_pose.POSE_CONNECTIONS,
                    landmark_drawing_spec=mp_drawing.DrawingSpec(color=(0, 255, 255), thickness=3, circle_radius=3),
                    connection_drawing_spec=mp_drawing.DrawingSpec(color=(0, 100, 255), thickness=3)
                    )
